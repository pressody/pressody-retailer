...
class Activation extends AbstractHookProvider {
	...
	private function create_cron_jobs() {
		wp_clear_scheduled_hook( 'pixelgradelt_retailer_cleanup_logs' );

		wp_schedule_event( time() + ( 3 * HOUR_IN_SECONDS ), 'daily', 'pixelgradelt_retailer_cleanup_logs' );
	}
	...
	private function create_or_update_tables() {
		...
		if ( self::DB_VERSION === get_option( 'pixelgradelt_retailer_dbversion' ) ) {
			return;
		}

		// Create/update the log table
		$log_table = "
CREATE TABLE {$wpdb->prefix}pixelgradelt_retailer_log (
  ...
  KEY level (level)
		...
		update_option( 'pixelgradelt_retailer_dbversion', self::DB_VERSION );
	}
	...
	public static function drop_tables() {
		...
		$wpdb->query( "DROP TABLE IF EXISTS {$wpdb->prefix}pixelgradelt_retailer_log" ); // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
	}
}
