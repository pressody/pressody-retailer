...
function* createApiKey( name, userId ) {
	...
	const result = yield apiFetch( {
		path: '/pixelgradelt_retailer/v1/apikeys',
		method: 'POST',
		data: {
			name,
			user: userId,
...
function* revokeApiKey( token, userId ) {
	...
	const result = yield apiFetch( {
		path: `/pixelgradelt_retailer/v1/apikeys/${ token }?user=${ userId }`,
		method: 'DELETE',
	} );
...
function* getApiKeys() {
	...
	const apiKeys = yield apiFetch( { path: `/pixelgradelt_retailer/v1/apikeys?user=${ userId }` } );
